...
## Points of attention
Consider other means of authentication. e.g. using app-based authentication provisioned through the [Initialize-PnPPowerShellAuthentication](https://docs.microsoft.com/en-us/powershell/module/sharepoint-pnp/initialize-pnppowershellauthentication) cmdlet. This would require a change to the Connect-PnPOnline part as follows:

`$connection = Connect-PnPOnline -Url "https://$tenantName-admin.sharepoint.com" -ClientId $credentials.UserName -Thumbprint $thumbPrint-Tenant "$tenantName.onmicrosoft.com" -ReturnConnection`
